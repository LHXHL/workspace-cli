...
| --- | --- | --- | --- |
| R-01 Configuration contract | The URL, API Key, timeout, and TLS interfaces are discoverable and consistent: `--url`/`TANSWER_URL`/`tanswer.url`; `--api-key`/`TANSWER_API_KEY`/`tanswer.api_key`; `--timeout`/`TANSWER_TIMEOUT`/`tanswer.timeout`; `--insecure`/`TANSWER_INSECURE`/`tanswer.insecure`. Configuration lookup is `flags > environment/.env > recognized ./config.yaml > ~/.chaitin-cli/config.yaml`. | `TestTAnswerRuntimeAndRepositoryGuidanceListAllConfigurationSources`; `TestCompanySkillMatchesRuntimeConfigDiscovery`; configuration unit tests. | Run `chaitin-cli tanswer --help` and `TANSWER_URL=http://127.0.0.1:9 TANSWER_API_KEY=placeholder TANSWER_TIMEOUT=7s TANSWER_INSECURE=true chaitin-cli --config /dev/null tanswer auth status`. |
